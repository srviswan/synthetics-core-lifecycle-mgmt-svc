...
CREATE TABLE Cashflow (
    ...
    cashflow_status VARCHAR(20) NOT NULL DEFAULT 'ACCRUAL' CHECK (cashflow_status IN ('ACCRUAL','SCHEDULED_PAYMENT','PAID_NOTSETTLED','SETTLED')),
    ...
    CONSTRAINT ck_cashflow_status_transition CHECK (
        (cashflow_status = 'ACCRUAL' AND status_transition_timestamp IS NULL) OR
        (cashflow_status IN ('SCHEDULED_PAYMENT','PAID_NOTSETTLED','SETTLED') AND status_transition_timestamp IS NOT NULL)
    )
);
...
CREATE INDEX IX_Cashflow_Status ON Cashflow(cashflow_status);
...
CREATE INDEX IX_Cashflow_SettlementEligible ON Cashflow(settlement_date, cashflow_status) WHERE cashflow_status = 'SCHEDULED_PAYMENT';
```

### Cashflow State Machine

**Cashflow Lifecycle States**:
```
ACCRUAL → SCHEDULED_PAYMENT → PAID_NOTSETTLED → SETTLED
```

**State Definitions**:
- **`ACCRUAL`**: Initial state when cashflow is generated but not yet ready for settlement
- **`SCHEDULED_PAYMENT`**: Cashflow is ready for settlement and sent to settlement system
- **`PAID_NOTSETTLED`**: Payment has been made but settlement confirmation is pending
- **`SETTLED`**: Final state - settlement is confirmed and complete

**State Transition Rules**:
```sql
-- Transition from ACCRUAL to SCHEDULED_PAYMENT (when settlement_date <= business_date)
UPDATE Cashflow 
SET cashflow_status = 'SCHEDULED_PAYMENT',
    status_transition_timestamp = GETDATE(),
    updated_timestamp = GETDATE()
WHERE cashflow_status = 'ACCRUAL' 
  AND settlement_date <= CAST(GETDATE() AS DATE)
  AND is_cancelled = 0;

-- Transition from SCHEDULED_PAYMENT to PAID_NOTSETTLED (when payment is made)
UPDATE Cashflow 
SET cashflow_status = 'PAID_NOTSETTLED',
    status_transition_timestamp = GETDATE(),
    settlement_reference = @settlement_ref,
    updated_timestamp = GETDATE()
WHERE cashflow_id = @cashflow_id 
  AND cashflow_status = 'SCHEDULED_PAYMENT';

-- Transition from PAID_NOTSETTLED to SETTLED (when settlement is confirmed)
UPDATE Cashflow 
SET cashflow_status = 'SETTLED',
    status_transition_timestamp = GETDATE(),
    updated_timestamp = GETDATE()
WHERE cashflow_id = @cashflow_id 
  AND cashflow_status = 'PAID_NOTSETTLED';
```

**Settlement Eligibility Query**:
```sql
-- Only SCHEDULED_PAYMENT status cashflows are sent to settlement system
SELECT cashflow_id, trade_id, payer_party_id, receiver_party_id, 
       currency, amount, settlement_date, payment_type
FROM Cashflow 
WHERE cashflow_status = 'SCHEDULED_PAYMENT'
  AND settlement_date <= CAST(GETDATE() AS DATE)
  AND is_cancelled = 0
ORDER BY settlement_date, trade_id;
```
