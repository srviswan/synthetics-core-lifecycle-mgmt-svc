...
public class ParallelProcessor {
     ...
     * @param positions List of positions to process
     * @param valuationDate The valuation date
     * @param originEventId The origin event ID
     * @return CompletableFuture containing all generated cashflows
     */
    public CompletableFuture<List<Cashflow>> processPositions(List<Position> positions, LocalDate valuationDate, String originEventId) {
        log.info("Starting parallel processing of {} positions", positions.size());
        LocalDateTime startTime = LocalDateTime.now();

        List<CompletableFuture<List<Cashflow>>> positionFutures = positions.stream()
            .map(position -> processPositionAsync(position, valuationDate, originEventId))
            .toList();

        return CompletableFuture.allOf(positionFutures.toArray(new CompletableFuture[0]))
            .thenApply(v -> {
                List<Cashflow> allCashflows = new ArrayList<>();
                for (CompletableFuture<List<Cashflow>> future : positionFutures) {
                    try {
                        ...
                        log.error("Error processing position future", e);
                    }
                }

                Duration processingTime = Duration.between(startTime, LocalDateTime.now());
                log.info("Completed parallel processing of {} positions in {} ms, generated {} cashflows",
                    positions.size(), processingTime.toMillis(), allCashflows.size());

                // Record metrics
                meterRegistry.counter("positions.processed", "batch_size", String.valueOf(positions.size()))
                    .increment(positions.size());
                meterRegistry.timer("positions.processing.time", "batch_size", String.valueOf(positions.size()))
                    .record(processingTime);
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                ...
                meterRegistry.timer("positions.processing.time", "position_id", position.getPositionId())
                    .record(processingTime);
                DistributionSummary.builder("positions.per.trade")
                    .tag("trade_id", position.getTradeId())
