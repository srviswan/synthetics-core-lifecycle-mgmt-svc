...
public class CashflowEngine {
    ...
    public List<Cashflow> generateCashflows(Trade trade, LocalDate valuationDate, String originEventId) {
        ...
        try {
            ...
            // Use parallel processing for multiple positions if available
            if (trade.getPositions().size() > 1) {
                log.info("Using parallel processing for {} positions in trade {}", trade.getPositions().size(), trade.getTradeId());
                try {
                    List<Position> positions = trade.getPositions();
                    List<Cashflow> parallelCashflows = parallelProcessor.processPositions(positions, valuationDate, originEventId).get();
                    cashflows.addAll(parallelCashflows);
                } catch (Exception e) {
                    log.warn("Parallel processing failed, falling back to sequential processing for trade: {}", trade.getTradeId(), e);
                ...
                // Use sequential processing for single position
                for (Position position : trade.getPositions()) {
                    List<Cashflow> positionCashflows = generatePositionCashflows(position, valuationDate, originEventId);
                    cashflows.addAll(positionCashflows);
                }
